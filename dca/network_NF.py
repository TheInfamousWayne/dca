...
from .helper import MaskingDense 
...
class Autoencoder():
    ...
    def build(self):
        ...
        for i, (hid_size, hid_drop) in enumerate(zip(self.hidden_size, self.hidden_dropout)):
            ...
            if i == centre_idx:
                z_sigma = []
                z_mean_0 = Dense(hid_size, activation=None, kernel_initializer=self.init, 
                                kernel_regularizer=l1_l2(l1, l2), 
                                name=layer_name+"_mean")(last_hidden)
                
                z_log_var_0 = Dense(hid_size, activation=None, kernel_initializer=self.init,
                                kernel_regularizer=l1_l2(l1, l2),
                ...
                z_0 = Lambda(sampling, name='z_0')([z_mean, z_log_var])
                
                # last_hidden = z # for passing throught the decoder model
                # IAF Flows
                self.masking_dim = 10 * hid_size
                h = Dense(hid_size, activation='sigmoid', kernel_initializer=self.init,
                                kernel_regularizer=l1_l2(l1, l2),
                                name=layer_name+"_h")(last_hidden)
                z_t = z_0
                for i in range(num_iaf):
                    reverse = (i > 0)
                    z_t, _z_sigma_t = make_iaf_layer(z_t, h, hid_size, reverse=reverse)
    ...
    def make_iaf_layer(prev_z, enc_h, latent_dim, reverse=False):
        if reverse:
            prev_z = Lambda(lambda x: K.reverse(x, axes=-1))(prev_z)
            
        iaf_in = [prev_z, enc_h] if enc_h is not None else prev_z
        m = MaskingDense(self.masking_dim, latent_dim, dropout_rate=0., hidden_layers=2,
