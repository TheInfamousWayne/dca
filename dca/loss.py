...
def poisson_loss(y_true, y_pred):
    ...
    ret = y_pred - y_true*tf.compat.v1.log(y_pred+1e-10) + tf.compat.v1.lgamma(y_true+1.0)
...
class NB(object):
    ...
    def loss(self, y_true, y_pred, mean=True):
        ...
        with tf.name_scope(self.scope):
            ...
            t1 = tf.compat.v1.lgamma(theta+eps) + tf.compat.v1.lgamma(y_true+1.0) - tf.compat.v1.lgamma(y_true+theta+eps)
            t2 = (theta+y_true) * tf.compat.v1.log(1.0 + (y_pred/(theta+eps))) + (y_true * (tf.compat.v1.log(theta+eps) - tf.compat.v1.log(y_pred+eps)))
...
class ZINB(NB):
    ...
    def loss(self, y_true, y_pred, mean=True):
        ...
        with tf.name_scope(self.scope):
            ...
            nb_case = super().loss(y_true, y_pred, mean=False) - tf.compat.v1.log(1.0-self.pi+eps)
            ...
            zero_case = -tf.compat.v1.log(self.pi + ((1.0-self.pi)*zero_nb)+eps)
